return {
  ...
  opts = function()
    ...
    local global_commands = {
      gen_cpp_class = function(state)
        local gen_internal = function(opts)
          local node = state.tree:get_node()
          if node.type == "directory" then
            vim.cmd((":!cmkr add cxx %s %s"):format(opts, node:get_id()))
          else
            vim.cmd((":!cmkr add cxx %s %s"):format(opts, node:get_parent_id()))
          end
        end
        local input = {}
        require("neo-tree.ui.inputs").input("Choose C++ file name: ", nil, gen_internal, input, true)
      end,
      gen_c_class = function(state)
        local gen_internal = function(opts)
          local node = state.tree:get_node()
          if node.type == "directory" then
            vim.cmd((":!cmkr add c %s %s"):format(opts, node:get_id()))
          else
            vim.cmd((":!cmkr add c %s %s"):format(opts, node:get_parent_id()))
          end
        end
        local input = {}
        require("neo-tree.ui.inputs").input("Choose C file name: ", nil, gen_internal, input, true)
